...
static void AddSearchWindow(u8 width)
{
    ...
    template = SetWindowTemplateFields(0, 1, y, width, 3, 14, 8);
...
#define WINDOW_COL_1        (WINDOW_COL_0 + (GetFontAttribute(sDexNavSearchDataPtr->windowId, FONTATTR_MAX_LETTER_WIDTH) * (POKEMON_NAME_LENGTH)) - 20)
#define WINDOW_MOVE_NAME_X  (WINDOW_COL_1 + (GetFontAttribute(sDexNavSearchDataPtr->windowId, FONTATTR_MAX_LETTER_WIDTH) * 6) - 0)
#define SEARCH_ARROW_X      (WINDOW_MOVE_NAME_X + 70) // +90
...
static bool8 DexNavPickTile(u8 environment, u8 areaX, u8 areaY, bool8 smallScan)
{
    ...
    s16 topX = gSaveBlock1Ptr->pos.x + SCANSTART_X + (smallScan * 4);
    s16 topY = gSaveBlock1Ptr->pos.y + SCANSTART_Y + (smallScan * 4);
    s16 botX = topX + areaX;
    s16 botY = topY + areaY;
    u8 i;
    bool8 nextIter;
    ...
    while (topY < botY)
    {
        while (topX < botX)
        {
            ...
            case ENCOUNTER_TYPE_WATER:
                if (MetatileBehavior_IsSurfable(tileBehaviour))
                {
        ...
        topY++;
        topX = gSaveBlock1Ptr->pos.x + SCANSTART_X + (smallScan * 4);
    }
...
static bool8 TryStartHiddenMonFieldEffect(u8 environment, u8 xSize, u8 ySize, bool8 smallScan)
{
    ...
    if (DexNavPickTile(environment, xSize, ySize, smallScan))
    {
        ...
        case ENCOUNTER_TYPE_LAND:
            ...
            {
                if (MetatileBehavior_UnusedIsTallGrass(metatileBehaviour)) //Grass in cave
                    fldEffId = FLDEFF_UNKNOWN_19; //FLDEFF_SHAKING_GRASS;
                else if (MetatileBehavior_IsLongGrass(metatileBehaviour)) //Really tall grass
                    fldEffId = FLDEFF_UNKNOWN_20; //FLDEFF_SHAKING_LONG_GRASS;
                else if (MetatileBehavior_IsSandOrDeepSand(metatileBehaviour))
                    fldEffId = FLDEFF_UNKNOWN_21;
                else
                    fldEffId = FLDEFF_CAVE_DUST;
            }
            else //outdoor, underwater
            {
                if (MetatileBehavior_UnusedIsTallGrass(metatileBehaviour)) //Regular grass
                    fldEffId = FLDEFF_UNKNOWN_19; //FLDEFF_SHAKING_GRASS;
                else if (MetatileBehavior_IsLongGrass(metatileBehaviour)) //Really tall grass
                    fldEffId = FLDEFF_UNKNOWN_20; //FLDEFF_SHAKING_LONG_GRASS;
...
//    LoadPalette(sHeldItemPalette, 0x100 + (16 * sHeldItemOam.paletteNum), 32); // this one breaks the palette for the shakinggrass field effects
    LoadCompressedSpriteSheetUsingHeap(&sPotentialStarSpriteSheet);
//    //LoadCompressedSpriteSheetUsingHeap(&sSightSpriteSheet);   //eye replaced with arrow // unused 
...
static void Task_InitDexNavSearch(u8 taskId)
{
    ...
    if (sDexNavSearchDataPtr->monLevel == MON_LEVEL_NONEXISTENT || !TryStartHiddenMonFieldEffect(sDexNavSearchDataPtr->environment, 10, 10, FALSE)) // 12, 12 at first, maybe too large?
    {
        Free(sDexNavSearchDataPtr);
        FreeMonIconPalettes();
...
bool8 TryStartDexnavSearch(void)
{
    ...
    DismissMapNamePopup(); //    HideMapNamePopUpWindow();
    ChangeBgY_ScreenOff(0, 0, 0); // ChangeBgY(0, 0, 0);
...
static void Task_DexNavSearch(u8 taskId)
{
    ...
    if (sDexNavSearchDataPtr->hiddenSearch && !task->tRevealed &&
        (JOY_NEW(SELECT_BUTTON) || (sDexNavSearchDataPtr->proximity < CREEPING_PROXIMITY)))
    {
        PlaySE(SE_DEX_SEARCH);
        ClearStdWindowAndFrameToTransparent(sDexNavSearchDataPtr->windowId, FALSE);
        CopyWindowToVram(sDexNavSearchDataPtr->windowId, 3);
        ...
        && task->tRevealed)
    {
        ...
        FieldEffectStop(&gSprites[sDexNavSearchDataPtr->fldEffSpriteId], sDexNavSearchDataPtr->fldEffId);
        while (1) {
            if (TryStartHiddenMonFieldEffect(sDexNavSearchDataPtr->environment, 8, 8, TRUE)) // 10, 10 at first, maybe too large?
                break;
        }
...
static void CreateDexNavWildMon(u16 species, u8 potential, u8 level, u8 abilityNum, u16 item, u16* moves)
{
    ...
    GenerateWildMon(species, level, 0);  // shiny rate bonus handled in CreateBoxMon // Gotta handle the Unowns here, hence the 0 at the end...
...
static void DexNavGenerateMoveset(u16 species, u8 searchLevel, u8 encounterLevel, u16* moveDst)
{
    ...
    GenerateWildMon(species, encounterLevel, 0); // Gotta handle the Unowns here, hence the 0 at the end...
...
static void Task_DexNavMain(u8 taskId)
{
    ...
    else if (JOY_NEW(R_BUTTON)) // Use the R button in dexnav GUI to register a species to search for
    {
        ...
        {            
            ...
            gSaveBlock1Ptr->registeredItem = ITEM_NONE; // set the registered item to ITEM_NONE
        }
        else
        {
            PlaySE(SE_FAILURE);
...
bool8 TryFindHiddenPokemon(void)
{
    ...
    if ((*stepPtr) == 0 && (Random() % 100 < HIDDEN_MON_SEARCH_RATE))
    {
        ...
        while (1) {
            if (TryStartHiddenMonFieldEffect(sDexNavSearchDataPtr->environment, 6, 6, TRUE)) // 8, 8 at first, maybe too large?
                break;
            if (++attempts > 20)
...
{
    if (gSaveBlock2Ptr->dexNavChain < DEXNAV_CHAIN_MAX)
